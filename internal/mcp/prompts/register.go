...
func RegisterAll(server *mcp.Server) {
	server.AddPrompt(&mcp.Prompt{
		...
		Arguments: []*mcp.PromptArgument{
			{Name: "project_id", Description: "The Nucleus project ID", Required: true},
...
func analyzeFindingPrompt(ctx context.Context, req *mcp.GetPromptRequest) (*mcp.GetPromptResult, error) {
	...
	return &mcp.GetPromptResult{
		...
		Messages: []*mcp.PromptMessage{
			{
				Role:    "user",
				Content: &mcp.TextContent{Text: fmt.Sprintf(`Analyze finding %s in Nucleus project %s.
...
func securityReportPrompt(ctx context.Context, req *mcp.GetPromptRequest) (*mcp.GetPromptResult, error) {
	...
	return &mcp.GetPromptResult{
		...
		Messages: []*mcp.PromptMessage{
			{
				Role:    "user",
				Content: &mcp.TextContent{Text: fmt.Sprintf(`Generate a security report for Nucleus project %s covering the last %s days.
...
func riskAssessmentPrompt(ctx context.Context, req *mcp.GetPromptRequest) (*mcp.GetPromptResult, error) {
	...
	return &mcp.GetPromptResult{
		...
		Messages: []*mcp.PromptMessage{
			{
				Role:    "user",
				Content: &mcp.TextContent{Text: fmt.Sprintf(`Perform a comprehensive risk assessment for Nucleus project %s.
...
func triageFindingsPrompt(ctx context.Context, req *mcp.GetPromptRequest) (*mcp.GetPromptResult, error) {
	...
	return &mcp.GetPromptResult{
		...
		Messages: []*mcp.PromptMessage{
			{
				Role:    "user",
				Content: &mcp.TextContent{Text: fmt.Sprintf(`Guide me through triaging findings in Nucleus project %s.
...
func nucleusReportPrompt(ctx context.Context, req *mcp.GetPromptRequest) (*mcp.GetPromptResult, error) {
	...
	return &mcp.GetPromptResult{
		...
		Messages: []*mcp.PromptMessage{
			{
				Role:    "user",
				Content: &mcp.TextContent{Text: fmt.Sprintf(`Generate a Nucleus Security status summary report for project %s.
...
1. Call list_teams with project_id=%s to get available teams.
...
- Prioritized recommendations`, projectID, projectID)},
			},
		},
	}, nil
}
